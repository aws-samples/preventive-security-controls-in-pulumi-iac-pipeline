...
1. The Security team defines the cloud security policies & controls and implements them as policy-as-code. This is maintained on a separate repo on the chosen version control service. AWS CodeCommit is used to store the policy-as-code in the sample provided. Every time the Security team pushes changes, a new version of the package needs to be uploaded to AWS CodeArtifact.
1. The DevOps team enforces the preventive checks as part of the buildspec file in the pipeline's CodeBuild project. The pre-build phase also covers other forms of security scanning.
1. The developers commit IaC code for deploying Cloud services and other resources in AWS, which triggers the build and the code is evaluated to measure compliance with built-in and custom policies.
...
1. Corresponding metrics are published in CloudWatch or other destination as part of the post-build step.
...
1. `<package_name>-<version_number>-py3-none-any.whl`
1. `<package_name>-<version_number>.tar.gz`

To upload the generated distribution archives, we use the [twine](https://pypi.org/project/twine/) package. The instructions are as follows:
...
This will upload the package to AWS CodeArtifact. This method allows for managing versions of the policy-as-code easily. To download the package and use it within the pipeline for policy enforcement, some additional commands are to be added into `buildspec.yml` for the pipeline. Specifically, these following command connects to the CodeArtifact repository and enable pip to download our custom package later. A sample buildspec file has been provided at `sample-code/sample-build-file/buildspec.yaml`.
...
The sample code for all the components highlighted in the list above are available under `sample-code/sample-resources/resources/` in the repository. Within `sample-code/sample-resources/checks/custom-policy-crossguard/requirements.txt`, add the name and version of the Python package that has been built and deployed to AWS CodeArtifact. The `pulumi preview` command later refers to this checks folder for all policies to verify the deployable resources.
...
The figure below depicts the steps within the `buildspec.yml` file:
...
**Note** - The sample code provided as part of this artifact is for demo purposes only and should not be used in production or non-production environments directly. The policy-as-code provided should also be thoroughly tested in the production environments before using in production.
...
- Status – Failure status *(Only failures are captured)*
- Author – the name of the Committer
- Email – the email of the Committer

The metrics.json file generated by the parser is provided as input to the `cloudwatch put-metric-data` command to export all the captured metrics to CloudWatch Metrics and visualize there. CloudWatch Alarms can also be provisioned to monitor the status, if needed, as an additional step.
...
This library is licensed under the MIT-0 License. See [LICENSE](LICENSE) for details.
